...
function createDirectiveWithFields(directiveName, fields, extra) {
    return (0, ast_builders_1.createDirectiveNode)(directiveName, __assign({ fields: (0, ast_builders_1.createStringValueNode)(fields) }, (extra || {})));
}
...
function isObjectConfigToDo(_a) {
    var extend = _a.extend, resolvable = _a.resolvable, keyFields = _a.keyFields;
        ...
        ObjectTypeDefinition: {
            enter: function (node) {
                ...
                if (objectTypesTodo.has(currentObjectName)) {
                    ...
                    var _a = federationConfig[currentObjectName], keyFields = _a.keyFields, extend = _a.extend, resolvable_1 = _a.resolvable;
                    var newDirectives = keyFields
                        ? keyFields.map(function (keyField) {
                            return createDirectiveWithFields('key', keyField, __assign({}, (resolvable_1 === false
                                ? { resolvable: (0, ast_builders_1.createBooleanValueNode)(resolvable_1) }
                                : {})));
                        })
                        : [];
                    return __assign(__assign({}, node), { directives: __spreadArray(__spreadArray([], (node.directives || []), true), newDirectives, true), kind: extend ? language_1.Kind.OBJECT_TYPE_EXTENSION : node.kind });
                }
            },
            leave: function () {
